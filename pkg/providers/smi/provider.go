...
func (p *Provider) BuildConfig() (*dynamic.Configuration, error) {
	...
	for _, service := range sevices {
		for _, endpoint := range service.endpoints {
			podSa := p[endpoint.address]

			s[podSa].annotations = service.annotations
			s[podSa].ports = service.ports
			s[podSa].selector = service.selector
			s[podSa].endpoints.append(endpoint.address)
		}
	}

	for _, target: range trafficTarget {
		serviceSrcs := s[target.sources]
		serviceDests := s[target.destination]
	...
	/*
	{
		...
		"serviceAccounts": {
	      "service-account-name": [services1, service2, service3]
	    },
	    "splits": {
			"name": {
	          "service": service1,
	...
	for _, service := range sevices {
		for _, endpoint := range service.endpoints {
			...
			s[podSa][name].annotations = service.annotations
			s[podSa][name].ports = service.ports
			s[podSa][name].selector = service.selector
			s[podSa][name].endpoints.append(endpoint.address)
		}
	}

	for _, target: range traffic target {
		serviceSrcs := s[target.sources]
		serviceDests := s[target.destination]
	...
	for _, pod := range pods {
		podsSa[pod.namespace][pod.serviceAccount] = Pod{}
	}

	for _, service := range services2 {
		services[service.name] = service
		for _, endpoint := range service.endpoints {
			servicesEndpoints[endpoint.address] = { annotation, port, selector, name }
		}
	}

	for _, target := range trafficTargets {
		// target.source => generate un whitelist middleware
			// -> endpoints = pods with service account mentionned
		sourcePods := podsSa[target.source.namespace][target.source.name]
		// target.destination => generate un load balancer
			// -> endpoints
		destPods := podsSa[target.destination.namespace][target.destination.name]

		servicesPods := make(map[string], []pods)
		for _, endpoint := destinationPods {
			svc := servicesEndpoints[endpoint.address]
			servicesPods[svc.name] = {svc, pods.append(endpoint.address) }
		}

		for name, { pods, svc } := range servicePods {
			middleware = [whitelist]
			// build middleware

		 	rules := name + "." + namespace + "maesh"
			router := Router(rules, middlewares)
			service := Service(pods)
		 	key := "plop"

		 	config.HTTP.Router[key] = router
			config.HTTP.Service[key] = service
		}
		//  whitelist, from sourcePods
		...
		// Rules1 = Host(serviceB.ns.maesh)
		// Rules2 = Host(serviceA.ns.maesh)

		// Router(rules1, middlewares1) => Service (load-balancer(destPods from serviceB))
		// Router(rules2, middlewares2) => Service (load-balancer(destPods from serviceA))
	}

	for _, split := range trafficSplits {
		svc := services[split.service]

		filteredBackends = []
		for _, backend := range split.backends {
			eps := getServiceEndpoints()
			for _, ep := range eps {
				sa := eps.serviceAccount
				if svc.sa can talk to sa {
					filteredBackends.push(backend)
				}
			}
		}

		rules = "Host() && trafficSplitRule"
		middleware = [whitelist]

		Router(rules, middlewares)
		//
	}

	// Router(Host() && match, middleware0) => Service(WRR) -> Router(middleware service1)
	//                                                      -> Router(middleware service2)

	/*
	    services: [{ annotation, selector, ports, name }]
		trafficTarget: [
	     {
	       name: "",
	       source: [{ ip, service }]
